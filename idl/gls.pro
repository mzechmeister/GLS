PRO GLS, f, p, time, ydata, dy=dy, fbeg=fbeg, fend=fend,  pbeg=pbeg, pend=pend, np=np, ls=ls, nostat=nostat, noplot=noplot, ofac=ofac

;+
; Name        : GLS
; Version     : v1.01 (2011/08/09)
;
...
;               IDL> gls, f,p, t,y
;
; Syntax      : IDL> gls, f, p, time, ydata, dy=dy,
...
IF KEYWORD_SET(dy) THEN w = 1./dy^2  ; use weights if given
...
IF NOT KEYWORD_SET(noplot) THEN BEGIN
   WINDOW,0 & PLOT, f,p, $                   ; periodogram
      xtitle="frequency", ytitle="power"
      OPLOT, f,p[i]+aa*(f-f[i])^2,color=250  ; peak curvature
   WINDOW,1 & PLOT, time,ydata,psym=4,$      ; time serie
      xtitle="time"
   t_x = MIN(time)+tbase*(FINDGEN(200*tbase/P[i])/(200*tbase/P[i]))
      OPLOT, t_x, Amp*sin(twopi*f[i]*(t_x-T0))+off[i]
